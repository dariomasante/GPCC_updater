.First <- function(){
  ...
  # netcdf_update
  netcdf_update = function(target_nc, what, yr, mm){
    ...
    target_nc = nc_open(target_nc, write=TRUE) # open netcdf
    tm = ncvar_get(target_nc, 'time')
    dy = floor(tm / 12)
    dm = tm - 12 * dy + 1; dm = ifelse(dm %in% 1:9, paste0('0', dm), dm)
    existingMonths = paste(1901 + dy, dm, sep='-') # !!Origin year date is hardcoded (January 1901)!!
    if(paste(yr,mm,sep='-') %in% existingMonths){
      ...
      if(conf){
        timeSlice = which(paste(yr,mm,sep='-') == existingMonths)
